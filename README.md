...
- A running session no longer ends from a local tap: countdown zero moves it into `awaitingNFCCompletion`, matching-tag scans can either complete the session or route manual exit into the precedent sheet, and an allowed exit closes safely without creating a main-chain node.  
  进行中的会话不再通过本地点按结束：倒计时归零后会进入 `awaitingNFCCompletion`，匹配标签的扫描要么真正完结会话，要么把主动退出送进判例裁决页，而被允许的退出会安全关闭且不会生成主链节点。
